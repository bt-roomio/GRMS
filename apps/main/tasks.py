...
def auto_check_out():
    ...
    guests = Guest.objects.filter(is_active=True, auto_check_out=True, check_out__lte=int(time.time()))
    ...
    rooms_with_active_guest_counts = Room.objects.annotate(
        active_guest_count=Count(
            "guests",
            filter=Q(guests__is_active=True, guests__auto_check_out=True, guests__check_out__lte=int(time.time())),
        )
