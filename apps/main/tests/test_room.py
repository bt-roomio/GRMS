...
class RoomTest(BaseTestCase):
    ...
    def test_list(self):
        ...
        self.assertEqual(response.data["results"][0]["number"], 102)
        self.assertEqual(response.data["results"][0]["floor"], "2")
        self.assertEqual(response.data["results"][0]["block"], "3")
        self.assertEqual(response.data["results"][1]["number"], 101)
        self.assertEqual(response.data["results"][1]["floor"], "1")
        self.assertEqual(response.data["results"][1]["block"], "1")
