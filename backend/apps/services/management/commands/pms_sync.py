"""
Django Management Command: PMS Reservation Sync
Manually sync reservations from PMS (Mews) for a specific time period
Publishes check-in/check-out events to RabbitMQ for processing
"""
...
class Command(BaseCommand):
    help = "Sync reservations from PMS (Mews) for a specific time period and publish to RabbitMQ"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"✓ Starting PMS sync for tenant: {config.tenant.title}"))
        ...
        try:
            ...
            logger.error(f"PMS sync error: {e}", exc_info=True)
