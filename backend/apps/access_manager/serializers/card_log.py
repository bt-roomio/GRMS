...
class CardLogSerializer(serializers.ModelSerializer):
    ...
    def get_public_spaces(self, obj):
        ...
        if obj.device:
            return [dps.public_space.name for dps in obj.device.device_public_spaces.select_related("public_space")]
        return []
...
class CardLogFilterParams(ValidatorSerializer):
    ...
    filters = serializers.DictField(
        required=False, child=serializers.DateTimeField(format="%Y-%m-%d %H:%M:%S", input_formats=["%Y-%m-%d %H:%M:%S"])
    )
