...
from core.utils.update_funvil_last_log import update_fanvil_last_log_id
...
def handle_card_event(device, value, ts_dt):
    try:
        ...
        fanvil_access_log_id = value.get("fanvil_access_log_id")
        is_fanvil = bool(fanvil_access_log_id)
        ...
        if not is_fanvil:
            ...
            update_fanvil_last_log_id(tenant_id, fanvil_access_log_id)
        ...
        if is_fanvil:
            additional_info.update(
                {
                    "fanvil_access_log_id": fanvil_access_log_id,
                    "open_type": value.get("open_type"),
                    "open_result": value.get("openResult"),
