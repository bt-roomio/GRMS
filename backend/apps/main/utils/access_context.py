...
def get_guest_access_context(guests: Union[Guest, List[Guest]]) -> dict:
    ...
    guest_public_spaces = PublicSpace.objects.filter(
        Q(guestpublicspace__guest__in=guests) | Q(room_type_public_spaces__room_type__room__guests__in=guests)
    ).distinct()

    door_lock_devices = (
        Device.objects.filter(as_door_lock_room__in=guest_rooms, is_active=True).select_related("tenant").distinct()
    )

    public_space_devices = (
        Device.objects.filter(
            device_public_spaces__public_space__in=guest_public_spaces,
            is_active=True,
        )
    ...
    return {
        ...
        "cards": cards,
        "blocked_guest_cards": blocked_guest_cards,
...
def get_staff_access_context(staff: Staff) -> dict:
    ...
    door_lock_devices = Device.objects.filter(as_door_lock_room__in=group_rooms, is_active=True).distinct()
    ...
    if door_lock_devices.exists():
        ...
        devices = (room_devices | public_space_devices).distinct()
    _logger.info(f"Devices for staff {staff.id}: {[device.id for device in devices]}")
